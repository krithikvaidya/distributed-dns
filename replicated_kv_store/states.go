...
func (node *RaftNode) ToCandidate() {
	...
	node.currentTerm++
	node.votedFor = node_meta.replica_id
	node.persistToStorage()
...
func (node *RaftNode) ToLeader() {
	...
	node.nextIndex = make([]int32, node_meta.n_replicas, node_meta.n_replicas)
	node.matchIndex = make([]int32, node_meta.n_replicas, node_meta.n_replicas)

	// initialize nextIndex, matchIndex
	for replica_id := int32(0); replica_id < node_meta.n_replicas; replica_id++ {

		if int32(replica_id) == node_meta.replica_id {
			continue
		}
