...
func TestInitializeNode(t *testing.T) {
	...
	// Check n_replicas
	if node.node_meta.n_replicas != no_of_replicas {
	...
	if node.node_meta.kvstore_addr != ":8081" {
		t.Errorf("Invalid value %v for kvstore_addr, expected :8081", node.node_meta.kvstore_addr)
	}
}
